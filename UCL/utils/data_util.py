...
def crop_pc(coord, feat, label, mask, split='train',
    ...
    coord -= coord.min(0) 
    return coord.astype(np.float32), feat.astype(np.float32) if feat is not None else None, label.astype(np.long) if label is not None else None, mask[crop_idx]

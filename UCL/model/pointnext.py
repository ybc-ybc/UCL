...
from model.layers import create_convblock1d, create_convblock2d, create_act, CHANNEL_MAP, \
...
class PointNextEncoder(nn.Module):
    ...
    def __init__(self, encoder_args):
        super().__init__()
        self.blocks = encoder_args.blocks
        self.strides = encoder_args.strides
        self.in_channels = encoder_args.in_channels
        self.aggr_args = encoder_args.aggr_args
        self.norm_args = encoder_args.norm_args
        self.act_args = encoder_args.act_args
        self.conv_args = encoder_args.conv_args
        self.sampler = 'fps'
        self.expansion = encoder_args.expansion
        self.sa_layers = encoder_args.sa_layers
        self.sa_use_res = encoder_args.sa_use_res
        self.use_res = True
        radius_scaling = 2
        nsample_scaling = 1
        radius = encoder_args.radius
        strides = encoder_args.strides
        width = encoder_args.width
        blocks = encoder_args.blocks
        group_args = encoder_args.group_args
        nsample = encoder_args.nsample
        ...
        for i in range(len(blocks)):
            ...
            encoder.append(self._make_enc(
                channels[i], blocks[i], stride=strides[i], group_args=group_args,
...
class PointNextDecoder(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        encoder_channel_list = decoder_args.encoder_channel_list
        decoder_layers = 2
        decoder_stages = 4
        ...
        if len(skip_channels) < decoder_stages:
            skip_channels.insert(0, decoder_args.in_channels)
